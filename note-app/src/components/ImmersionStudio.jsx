...
export default function ImmersionStudio() {
  ...
  const generateAITranscript = async (videoUrl) => {
    ...
    try {
      ...
      const baseUrl = import.meta.env.VITE_API_URL || (window.location.origin === 'null' || window.location.origin.includes('file://') ? 'http://localhost:3000' : window.location.origin);
